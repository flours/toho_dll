...
NormalEnemy::NormalEnemy(float x, float y, std::shared_ptr<Player> player) : AbstractEnemy(x, y, player)
{
	_movePatternID = 3;
	_shotPatternID = 0;
	_hp = 1;
...
void NormalEnemy::draw() const
{
	...
	int handle = Image::getIns()->getEnemyNormal()[add + imgID[(_counter / 8) % 4]];
...
{
	cv::Mat _image = Image::getIns()->idToMat(Image::getIns()->getEnemyNormal()[0]);
	_width = _image.cols;
	_height = _image.rows;
}